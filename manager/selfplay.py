...
class Play(object):
    def __init__(self):
        ...
        self.features = np.reshape([], (-1, BOARD_SIZE ** 2 * channel))  # one turn has 64 + 1
...
class SelfPlay(object):
    ...
    def __init__(self, player1='defender', player2='defender'):
        ...
        try:
            ...
            self.player1.model.predict(np.random.random((1, BOARD_SIZE, BOARD_SIZE, channel)))  # initialize the generator to accelerate the simulation
            self.player2.model.predict(np.random.random((1, BOARD_SIZE, BOARD_SIZE, channel)))
        finally:
            model_lock.release()
